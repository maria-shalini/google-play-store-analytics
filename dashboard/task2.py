...
def task2_dual_axis_chart(visible_mode=False):
    ...
    if base_filtered.empty:
        ...
        return None
    paid_apps = base_filtered[
        (base_filtered["Type"] == "Paid") &
        (base_filtered["Revenue"] >= 10000)
    ]

    free_apps = base_filtered[base_filtered["Type"] == "Free"]

    filtered_df = pd.concat([free_apps, paid_apps])
    ...
    summary_df = summary_df.reindex(["Free", "Paid"])
